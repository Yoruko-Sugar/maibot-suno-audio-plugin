...
TOOL_DESCRIPTION = (
    "【唯一产物】可独立发送的歌曲、纯音乐、歌词或短音效。调用后提交后台任务，完成后自动发送到当前聊天。\n\n"
    "【调用前提】用户最终明确想得到独立音频或歌词，例如写/生成一首歌、纯音乐、配乐文件、歌词、环境音或短音效。"
    "必须依据最终产物判断，不能只因出现‘声音、唱歌、音乐、生成、制作’等词调用。\n"
    "【与其他媒体工具的边界】静态专辑封面、歌曲配图交给生图工具；视频、MV、让图片动起来或唱歌的视频交给视频工具。"
    "‘画一张专辑封面’不是音频；‘猫唱歌的视频’不是独立歌曲；‘为这个视频生成一首可单独发送的配乐’才是音频。"
    "用户只是要求视频自带声音时不要调用本工具，由视频工具设置 generate_audio=true。\n"
    "【歧义处理】‘做一段雨夜氛围’等未说明最终媒介的请求不要调用，应先询问要图片、视频还是声音。"
    "‘写首歌并制作 MV’等多个独立付费产物不得同时创建任务，应先确认分步执行。默认一条消息只调用一个付费媒体生成工具。\n"
    "【操作选择】music=普通歌曲或音乐；instrumental=明确不要人声的纯音乐；custom_song=用户提供指定歌词的歌曲；"
    "lyrics=只写歌词、不生成音频；sound=短音效或环境音。触发消息 ID 必须来自当前提出请求的用户消息。\n"
    "【禁止调用】只讨论或评价音乐；询问能力；只有声音场景描述但没有生成要求；假设、否定或转述请求；当前消息已创建任务。"
)
...
TOOL_PARAMETERS = {
    "operation": {
        ...
        "description": "按用户最终想得到的独立音频或歌词类型选择；不得用于视频内声音轨道。",
        "required": True,
    },
    "description": {
        "type": "string",
        "description": "完整保留用户对独立歌曲、音乐、歌词或音效的要求。",
        "required": True,
    },
    "msg_id": {
        "type": "string",
